...
public interface CategoryParentRepo extends JpaRepository<CategoryParent, Long> {
	...
	@Query(value = "SELECT * FROM category_parents WHERE isActive = ?1 AND name LIKE %?2%", nativeQuery = true)
	List<CategoryParent> findBykeyword(Boolean isActive, String keyword);
}
